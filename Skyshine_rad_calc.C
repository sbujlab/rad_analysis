...
void Skyshine_rad_calc(string modifier = "thickness"){//,string units = "mm",int number = 6){
...
//use parserHallrad format to loop over files instead of doing it here
  ...
  file_in = TFile::Open(Form("%s_hallRad.root",modifier.c_str()));
  ...
  Double_t Flux_101_error = sqrt(Flux_101/nEvents);//9M events is the number per run that the averages normalize to
...
//}
  ...
  if (units!=ben){ //case = doing parameter space searches
    ...
    NEIL_101_out<<modifier<<","<<units<<","<<number<<","<<NEIL_101_ratio<<","<<NEIL_101_ratio_error<<std::endl;
    Flux_101_out<<modifier<<","<<units<<","<<number<<","<<Flux_101_ratio<<","<<Flux_101_ratio_error<<std::endl;
    Energy_101_out<<modifier<<","<<units<<","<<number<<","<<Energy_101_ratio<<","<<Energy_101_ratio_error<<std::endl;
  }
  else{ //case = benchmark
